...
def create_capsule(num_maps, height, width, all_joints, threshold, stride):
    """
    Creates capsule maps for all connections. One connection requires
    4 maps because of capsule parameters include center(x,y), length l.
    For coco we have 19 connections -> x2 it gives 57 maps

    :param num_maps: number of maps. for coco dataset we have 19 connections
    :param height: height dimension of the network output
    :param width: width dimension of the network output
    :param all_joints: list of all joints (for coco: 18 items)
    :param threshold: parameter determines the "radius" of a capsule
    :param stride: parameter used to scale down the coordinates of joints. Those coords
            relate to the original image size
    :return: capsule maps (height x width x 3*num_maps)
    """
    capsulemap = np.zeros((height, width, num_maps*3),dtype=np.float64)
    countmap = np.zeros((height, width, num_maps),dtype=np.uint8)
    for joints in all_joints:
        for plane_idx,(j_idx1,j_idx2) in enumerate(JointsLoader.joint_pairs):
            center_from = joints[j_idx1]
            ...
            _put_capsule_on_plane(capsulemap, countmap, plane_idx, x1, y1, x2, y2,
                              threshold, height, width)

    return capsulemap


def _put_capsule_on_plane(capsulemap, countmap, plane_idx, x1, y1, x2, y2,
                              threshold, height, width):
    ...
    #normalizaion for output range [0,1]
    distmax = math.sqrt(height ** 2 + width ** 2)                     
    normdist = norm / distmax 
    normx = x2 / width
    normy = y2 / height
    ...
    for y in range(min_y, max_y):
        for x in range(min_x, max_x):
            ...
            if cnt == 0:
                capsulemap[y][x][plane_idx * 3 + 0] = normx
                capsulemap[y][x][plane_idx * 3 + 1] = normy
                capsulemap[y][x][plane_idx * 3 + 2] = normdist
            else:#temporaty
                capsulemap[y][x][plane_idx * 3 + 0] = normx
                capsulemap[y][x][plane_idx * 3 + 1] = normy
                capsulemap[y][x][plane_idx * 3 + 2] = normdist

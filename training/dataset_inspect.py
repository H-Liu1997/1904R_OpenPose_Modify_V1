...
from training.dataset import read_img, gen_mask, augment, apply_mask, \
    create_all_mask, ALL_CAPSULE_MASK, ALL_PAF_MASK
from training.label_maps import create_heatmap, create_paf, create_capsule
...
def display_image(img, capsulemap, vectmap):
    """
    ...
    a.set_title('capsulemap')
    plt.imshow(_get_bgimg(img, target_size=(capsulemap.shape[1], capsulemap.shape[0])), alpha=0.5)
    ...
    plt.imshow(tmp2_odd, cmap=plt.cm.viridis, alpha=0.5)
    plt.colorbar()

    a = fig.add_subplot(3, 2, 3)
    a.set_title('paf-y')
    plt.imshow(_get_bgimg(img, target_size=(vectmap.shape[1], vectmap.shape[0])), alpha=0.5)
    plt.imshow(tmp2_even, cmap=plt.cm.viridis, alpha=0.5)
    plt.colorbar()
    
    tmp3 = capsulemap.transpose((2, 0, 1))
    tmp3_X = np.amax(np.absolute(tmp3[::3, :, :]), axis=0)
    tmp3_Y = np.amax(np.absolute(tmp3[1::3, :, :]), axis=0)
    tmp3_L = np.amax(np.absolute(tmp3[2::3, :, :]), axis=0)
    
    a = fig.add_subplot(3, 2, 4)
    a.set_title('cap-x')
    plt.imshow(_get_bgimg(img, target_size=(capsulemap.shape[1], capsulemap.shape[0])), alpha=0.5)
    plt.imshow(tmp3_X, cmap=plt.cm.viridis, alpha=0.5)
    plt.colorbar()
    
    a = fig.add_subplot(3, 2, 5)
    a.set_title('cap-y')
    plt.imshow(_get_bgimg(img, target_size=(capsulemap.shape[1], capsulemap.shape[0])), alpha=0.5)
    plt.imshow(tmp3_Y, cmap=plt.cm.viridis, alpha=0.5)
    plt.colorbar()
    
    a = fig.add_subplot(3, 2, 6)
    a.set_title('cap-l')
    plt.imshow(_get_bgimg(img, target_size=(capsulemap.shape[1], capsulemap.shape[0])), alpha=0.5)
    plt.imshow(tmp3_L, cmap=plt.cm.viridis, alpha=0.5)
    plt.colorbar()
...
def show_image_capsulemap_paf(g):
    """
    ...
    capsulemap = g[4].astype(np.float32)

    display_image(img, capsulemap, paf)
...
def build_debug_sample(components):
    ...
    if meta.mask is None:
       ...
       # mask_heatmap = ALL_HEATMAP_MASK
        mask_capsule = ALL_CAPSULE_MASK
    else:
        mask_paf = create_all_mask(meta.mask, 38, stride=8)
        mask_capsule = create_all_mask(meta.mask, 57, stride=8)

    capsulemap = create_capsule(JointsLoader.num_connections, 46, 46,
                        meta.aug_joints, 1, stride=8)

    pafmap = create_paf(JointsLoader.num_connections, 46, 46,
                           meta.aug_joints, 1, stride=8)

    return [meta, mask_paf, mask_capsule, pafmap, capsulemap]


if __name__ == '__main__':
    batch_size = 10
    curr_dir = os.path.dirname(__file__)
    annot_path = os.path.join(curr_dir, '/home/liuhaiyang/Document/keras_Realtime_Multi-Person_Pose_Estimation-master/keras_Realtime_Multi-Person_Pose_Estimation-master/dataset/annotations/person_keypoints_val2014.json')
    img_dir = os.path.abspath(os.path.join(curr_dir, '/home/liuhaiyang/Document/keras_Realtime_Multi-Person_Pose_Estimation-master/keras_Realtime_Multi-Person_Pose_Estimation-master/dataset/val2014/'))
    df = CocoDataFlow((368, 368),
                      COCODataPaths(annot_path, img_dir))#, select_ids=[1000])
    df.prepare()
    df = MapData(df, read_img)#nochange
